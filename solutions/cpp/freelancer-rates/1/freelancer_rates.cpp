...
int monthly_rate(double hourly_rate, double discount) {
    ...
    // discount.
    double monthly = hourly_rate * 176.0;
    
    double disc = apply_discount(monthly, discount);
    
    return static_cast<int>(std::ceil(disc));
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    double raw_daily_rate = daily_rate(hourly_rate);
    
    double discounted_daily_rate = apply_discount(raw_daily_rate, discount);
